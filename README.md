...
| [`apps/python/vaxcheck`](apps/python/vaxcheck/) | Python | School immunisation consent and screening calls: one CALL-E task per student captures consent, allergy history, prior doses and whether the family wants the free school session or their own doctor, then fail-closed triage clears a child only when every signal is unambiguous and routes every severe allergy, unclear history, unanswered question or low-confidence call to a nurse. Silence is never consent, idempotency keys are derived from school, date and student id so a re-run cannot phone a family twice, and `--preflight` validates a roster's region corridor through real `plan_call` requests without dialling. Fixture replay by default. |
